...
<body class="bg-gray-100">
    ...
    <div id="edit-enrolment-modal" class="modal-overlay">
        <div class="modal-content">
            ...
            <input type="hidden" id="edit-student-offered-course-id">
            <input type="hidden" id="edit-current-section-id">
    ...
    <script>
        ...
        function renderEnrolments() {
            ...
            filteredEnrolments.forEach(enrolment => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${enrolment.semester}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <button onclick="openEditEnrolmentModal(${enrolment.student_offered_course_id}, '${enrolment.name}', '${enrolment.regNo}', '${enrolment.semester}', ${enrolment.section_id})" 
                                class="action-btn edit">✏️ Edit</button>
                        <button onclick="openDeleteConfirmationModal(${enrolment.student_offered_course_id}, '${enrolment.name}', '${document.getElementById('course-title').textContent}', '${document.getElementById('course-section').textContent}')" 
                                class="action-btn delete">🗑️ Delete</button>
                    </td>
        ...
        async function loadAllSections() {
            try {
                ...
                if (data) {
                    ...
                    // Enable section dropdown if course is selected
                    if (document.getElementById('enrolment-course-dropdown').value) {
                        sectionDropdown.disabled = false;
                    }
                }
            } catch (error) {
        ...
        // Load sections for a specific offered course (for edit modal)
        async function loadSectionsForOfferedCourse(offeredCourseId, currentSectionId) {
            try {
                API_BASE_URL = await getApiBaseUrl();
                const response = await fetch(`${API_BASE_URL}api/Insertion/enrollment/sections/${offeredCourseId}`);
                const data = await response.json();
                ...
                if (data && data.status === 'success') {
                    ...
                    if (sectionDropdown.options.length === 1) { // Only the default option
                        sectionDropdown.disabled = true;
                        showMessage('edit-enrolment-message', 'No other sections available for this course', 'info');
                    } else {
                        sectionDropdown.disabled = false;
                    }
                } else {
                    showMessage('edit-enrolment-message', 'Failed to load sections for this course', 'error');
                }
            } catch (error) {
                console.error('Error loading sections for offered course:', error);
                showMessage('edit-enrolment-message', 'Error loading sections for this course', 'error');
            }
        }
        ...
        function loadEnrolmentCourseSections() {
            ...
            // Load all sections from the API (as requested)
            if (allSections.length === 0) {
                loadAllSections();
            } else {
                document.getElementById('enrolment-section-dropdown').disabled = false;
            }
        ...
        async function addEnrolment() {
            ...
            try {
                ...
                // Get the offered_course_id from the first section (if exists)
                let offeredCourseId = null;
                if (course.sections && course.sections.length > 0) {
                    offeredCourseId = course.sections[0].offered_course_id;
                }
                
                if (!offeredCourseId) {
                    showMessage('add-enrolment-message', 'Could not determine offered course. Please offer this course first.', 'error');
                    return;
                }
        ...
        // Open edit enrolment modal
        async function openEditEnrolmentModal(studentOfferedCourseId, name, regNo, semester, currentSectionId) {
            document.getElementById('edit-student-name').value = name;
            document.getElementById('edit-reg-no').value = regNo;
            document.getElementById('edit-semester').value = semester;
            document.getElementById('edit-student-offered-course-id').value = studentOfferedCourseId;
            document.getElementById('edit-current-section-id').value = currentSectionId;
            
            // Load sections for this offered course (excluding current section)
            await loadSectionsForOfferedCourse(studentOfferedCourseId, currentSectionId);
            ...
            try {
                ...
                if (data.status === 'success') {
                    ...
                    setTimeout(async () => {
                        ...
                        closeModal('edit-enrolment-modal');
                    }, 1500);
                } else {
                    showMessage('edit-enrolment-message', data.message || 'Failed to update enrolment.', 'error');
                }
        ...
        function showMessage(elementId, message, type) {
            ...
            if (type === 'success') {
                ...
                element.className = element.className.replace(/text-red-500/g, '').replace(/text-green-500/g, '') + ' text-blue-500';
            }
        }

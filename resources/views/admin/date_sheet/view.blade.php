...
<body>
    ...
    <div class="container">
        ...
        <div id="session-info" class="session-info">Session: Loading...</div>
        ...
        <div id="date-sheet-container">
            <!-- Will be populated dynamically -->
            <div class="text-center no-data">Loading date sheet...</div>
        </div>
    </div>
    ...
    <script>
            ...
            try {
                ...
                if (data.error) {
                    ...
                    document.getElementById('date-sheet-container').innerHTML = 
                        `<div class="text-center no-data">${data.error}</div>`;
                    return;
                }
                ...
                // Update UI
                document.getElementById('session-info').textContent = `Session: ${dateSheetData.SESSION || 'N/A'}`;
                populateTimeSlotDropdown();
                filterDateSheet();
            } catch (error) {
                console.error("Error loading date sheet:", error);
                document.getElementById('date-sheet-container').innerHTML = 
                    `<div class="text-center no-data">Failed to load date sheet. Please try again.</div>`;
            }
        }
        ...
        function populateTimeSlotDropdown() {
            ...
            ['Mid', 'Final'].forEach(type => {
                if (dateSheetData[type]) {
                    Object.keys(dateSheetData[type]).forEach(slot => {
                        timeSlots.add(slot);
                    });
                }
            });
            ...
            Array.from(timeSlots).sort().forEach(slot => {
                timeSlotMap[slot] = `Slot ${slotCount}`;
        ...
        function filterDateSheet() {
            ...
            if (examType === 'all') {
                if (dateSheetData.Mid) filteredData.Mid = {...dateSheetData.Mid};
                if (dateSheetData.Final) filteredData.Final = {...dateSheetData.Final};
            } else {
            ...
            if (timeSlot !== 'all') {
                Object.keys(filteredData).forEach(type => {
                    Object.keys(filteredData[type]).forEach(slot => {
                        if (slot !== timeSlot) {
                            delete filteredData[type][slot];
                        }
                    });
                });
            }
            
            renderDateSheet(filteredData);
        }
        
        function renderDateSheet(data) {
            const container = document.getElementById('date-sheet-container');
            container.innerHTML = '';
            
            let hasData = false;
            
            // Process Mid and Final exams
            ['Mid', 'Final'].forEach(type => {
                if (data[type] && Object.keys(data[type]).length > 0) {
                    hasData = true;
                    
                    Object.keys(data[type]).sort().forEach(slot => {
                        ...
                        slotHeader.textContent = `${type} Term - ${slot}`;
                        ...
                        slotContainer.appendChild(table);
            ...
            if (!hasData) {
                container.innerHTML = '<div class="text-center no-data">No date sheet data found for the selected filters.</div>';
            }
        }
